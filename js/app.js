/**
 * MAIN APP ORCHESTRATOR — v2.6.0
 * Updated for Official Diyanet API (emushaf.net)
 */

document.addEventListener('DOMContentLoaded', () => {

  // Force clean stale localStorage on v2.6.0 update
  if (localStorage.getItem('maarif_ver') !== '2.6.0') {
    localStorage.removeItem('maarif_city');
    localStorage.setItem('maarif_ver', '2.6.0');
  }
  ...
  function updateClock() {
    ...
    digitalClockTime.textContent = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;

    if (prayerData) updatePrayerState(hours, minutes, seconds);
  }
  ...
  function updateDateFields() {
    ...
    const hizirStart = new Date(year, 4, 6);
    const kasimStart = new Date(year, 10, 8);
    ...
    if (now >= hizirStart && now < kasimStart) {
      ...
      let count = 0;
      if (now >= kasimStart) count = Math.floor((now - kasimStart) / oneDay) + 1;
      else count = Math.floor((now - new Date(year - 1, 10, 8)) / oneDay) + 1;
    ...
    dayLengthChange.textContent = (now >= summerSolstice && now < winterSolstice) ? '📉 Günler kısalıyor' : '📈 Günler uzuyor';
  }

  function updatePrayerState(h, m, s) {
    const currentMins = h * 60 + m;
    const currentSecsTotal = h * 3600 + m * 60 + s;
    const timeToMins = (tStr) => { if (!tStr) return 0; const [ph, pm] = tStr.split(':').map(Number); return ph * 60 + pm; };
    ...
    document.querySelectorAll('.prayer-row').forEach((el, idx) => {
      ...
      if (idx === activeIndex) { el.classList.add('active'); if (badge) badge.textContent = 'AKTİF VAKİT'; }
      else { el.classList.remove('active'); if (badge) badge.textContent = (idx < activeIndex) ? 'Geçti' : 'Girecek'; }
    });

    let targetSecs = nextPrayer.mins * 60;
    if (targetSecs <= currentSecsTotal) targetSecs += 24 * 3600;
    const diffSecs = targetSecs - currentSecsTotal;
    const cdH = Math.floor(diffSecs / 3600), cdM = Math.floor((diffSecs % 3600) / 60);
    if (nextPrayerCountdown) nextPrayerCountdown.textContent = `⏳ ${nextPrayer.name} vaktine ${cdH > 0 ? cdH + ' Saat ' : ''}${cdM} Dk kaldı`;

    const dayLengthMins = prayerTimesMins[4].mins - prayerTimesMins[1].mins;
    const nightLengthMins = (1440) - dayLengthMins;
    dayDuration.textContent = `Gündüz Süresi: ${Math.floor(dayLengthMins / 60)} Saat ${dayLengthMins % 60} Dakika`;
    nightDuration.textContent = `Gece Süresi: ${Math.floor(nightLengthMins / 60)} Saat ${nightLengthMins % 60} Dakika`;
  }

  async function loadAllData() {
    const now = new Date();
    let lat = 39.9334, lon = 32.8597, diyanetId = 506, cityName = 'Ankara';

    if (currentCity === 'AUTO') {
      const loc = await window.ApiService.detectLocation();
      lat = loc.lat; lon = loc.lon; cityName = loc.name; diyanetId = loc.diyanetId;
    } else if (window.ApiService.CITIES[currentCity]) {
      const c = window.ApiService.CITIES[currentCity];
      lat = c.lat; lon = c.lon; cityName = c.name; diyanetId = c.diyanetId;
    }
    ...
    prayerData = await window.ApiService.fetchPrayerTimes(lat, lon, diyanetId);
  ...
  function startPixelShift() {
    setInterval(() => {
      if (!burninEnabled) { appRoot.style.transform = 'translate(0px, 0px)'; return; }
      appRoot.style.transform = `translate(${Math.floor(Math.random() * 5) - 2}px, ${Math.floor(Math.random() * 5) - 2}px)`;
    }, 60000);
  }

  selectCity.value = currentCity;
  burninToggle.checked = burninEnabled;
  const updateBurninBadge = () => burninBadge.style.display = burninEnabled ? 'inline-block' : 'none';
  updateBurninBadge();
  ...
  selectCity.addEventListener('change', (e) => { currentCity = e.target.value; localStorage.setItem('maarif_city', currentCity); loadAllData(); });
  burninToggle.addEventListener('change', (e) => { burninEnabled = e.target.checked; localStorage.setItem('maarif_burnin', burninEnabled); updateBurninBadge(); if (!burninEnabled) appRoot.style.transform = 'translate(0px, 0px)'; });
  ...
  btnModalFullscreen.addEventListener('click', () => { toggleFullscreen(); modal.classList.add('hidden'); });
